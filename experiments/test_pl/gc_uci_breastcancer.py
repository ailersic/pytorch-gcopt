...
class DemoClassDataset(Dataset):
    def __init__(self, transform=None):
        ...
        # data
        self.X = breast_cancer_wisconsin_diagnostic.data.features.to_numpy()
        self.y = np.squeeze(breast_cancer_wisconsin_diagnostic.data.targets.to_numpy()) == 'M'
...
class DemoClass(ContNet):
    def __init__(self, loglambda0: float, cont_lr: float, cont_reg: float, warmup_epochs: int):
        ...
        self.datasetname = 'UCI_BreastCancer'
        self.epochs = 100
        ...
        layers = []
        layers.extend([nn.Linear(30, self.h_dim), activ])
        for _ in range(self.n_layers - 2):
            layers.extend([nn.Linear(self.h_dim, self.h_dim), activ])
        layers.append(nn.Linear(self.h_dim, 2))
    ...
    def train_dataloader(self):
        return DataLoader(self.data_train, batch_size=25, shuffle=True, num_workers=16, persistent_workers=True)
        
    def val_dataloader(self):
        return DataLoader(self.data_val, batch_size=25, num_workers=16, persistent_workers=True)
